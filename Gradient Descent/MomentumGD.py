...
velocity_m = 0  # Initialize velocity for m
velocity_c = 0  # Initialize velocity for c
...
for i in range(num_iterations):
    ...
    velocity_m = momentum * velocity_m - learning_rate * grad_m  # Update velocity for m
    velocity_c = momentum * velocity_c - learning_rate * grad_c  # Update velocity for c
    m_current += velocity_m  # Update m
    c_current += velocity_c  # Update c
...
# Plot the path of gradient descent with momentum on the surface
...
fig.update_layout(
    ...
    title='Error Surface and Gradient Descent with Momentum Path'
)
...
plt.ylabel('Error')
plt.title('Error over Iterations during Gradient Descent with Momentum')
plt.show()